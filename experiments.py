...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help=f"Predictor model index (0={PREDICTOR_MODEL_NAMES[0]}, 1={PREDICTOR_MODEL_NAMES[1]}, ...)",
    )
    # --- Common Arguments ---
    parser.add_argument(
    ...
    # --- 3. Train Predictor and Build Test Subject ---
    predictor_func = None
    if stype in ["PLBF", "FastPLBF", "LBF"]:
        ...
        y_train_pred = train_pred_data["binary_label"]
        predictor_func = PREDICTOR_MODEL_FACTORIES[model_idx](
            X_train_pred,
            y_train_pred,
            DEVICE,
        )
    ...
    if all_results:
        ...
        if (
            ...
            if predictor_func and stype != "BF":
                predictor_size_bytes = deep_sizeof(predictor_func)  # Approximation
                print(
